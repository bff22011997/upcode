...
- (void)dragTableHeaderDidTriggerRefresh:(DragTableHeaderView_ot*)view
{
    if (self.dragFooterView.isLoading)
    {
        ...
        if ([self.dragDelegateFix respondsToSelector:@selector(dragTableLoadMoreCanceled:)])
        {
            [self.dragDelegateFix dragTableLoadMoreCanceled:self];
        }
    }
...
- (void)dragTableFooterDidTriggerLoadMore:(DragTableFooterView_ot *)view
{
    if (self.dragHeaderView.isLoading)
    {
        ...
        if ([self.dragDelegateFix respondsToSelector:@selector(dragTableRefreshCanceled:)])
        {
            [self.dragDelegateFix dragTableRefreshCanceled:self];
        }
    }
...
@dynamic dragDelegateFix;

- (id<UITableViewDragLoadDelegate>)dragDelegateFix
{
...
- (void)setDragDelegate:(id<UITableViewDragLoadDelegate>)dragDelegateFix refreshDatePermanentKey:(NSString *)refreshDatePermanentKey
{
    objc_setAssociatedObject(self, DRAG_DELEGATE_KEY, dragDelegateFix, OBJC_ASSOCIATION_ASSIGN);
    if (dragDelegateFix)
    {
...
- (void)callDelegateDidTriggerRefresh
{
    if ([self.dragDelegateFix respondsToSelector:@selector(dragTableDidTriggerRefresh:)])
    {
        [self.dragDelegateFix dragTableDidTriggerRefresh:self];
    }
}

- (void)callDelegateDidTriggerLoadMode
{
    if ([self.dragDelegateFix conformsToProtocol:@protocol(UITableViewDragLoadDelegate)])
    {
        [self.dragDelegateFix dragTableDidTriggerLoadMore:self];
    }
}
